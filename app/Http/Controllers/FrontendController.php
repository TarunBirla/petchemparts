...
public function manufacturerData(Request $request)
{
    ...
    $products = Product::where('status','active')
        ...
        // Manufacturer filter
        ->when($request->manufacturer_id, function ($q) use ($request) {
            $q->where('manufacturer_id', $request->manufacturer_id);
        })

        // ✅ MAIN CATEGORY FILTER (cat_id)
        ->when($request->category_id, function ($q) use ($request) {
            $q->where('cat_id', $request->category_id);
        })

        // ✅ SUB CATEGORY FILTER (child_cat_id)
        ->when($request->subcategory_id, function ($q) use ($request) {
            $q->where('child_cat_id', $request->subcategory_id);
        })
