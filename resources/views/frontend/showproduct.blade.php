...
<style>
/* ============================================================
   ALL PRODUCTS — identical UI to the Shop page
...
<div class="shp-hero">
    <div class="shp-hero-inner">
        <div>
            ...
            <p>{{ $totalProducts }} PARTS LISTED · VERIFIED BY OUR ENGINEERING DESK</p>
        </div>
        ...
        <aside class="shp-sidebar" id="shpSidebar">
            ...
            <div class="shp-widget">
                ...
                <ul class="shp-cat-list">
                    <li>
                        <a href="{{ url()->current() }}" class="{{ !request('search') && !request('min_price') ? 'is-active' : '' }}">
        ...
        <div class="shp-main">
            ...
            <div class="shp-search-box">
                ...
                <div class="shp-active-pills">
                    ...
                    <span class="shp-pill">
                        "{{ request('search') }}"
                        <a href="{{ url()->current() }}?{{ http_build_query(array_diff_key(request()->all(), ['search'=>''])) }}">✕</a>
                    </span>
                    @endif
                    @if(request('min_price') || request('max_price'))
                    <span class="shp-pill">
                        £{{ request('min_price', 0) }} – £{{ request('max_price', '∞') }}
                        <a href="{{ url()->current() }}?{{ http_build_query(array_diff_key(request()->all(), ['min_price'=>'','max_price'=>''])) }}">✕</a>
                    </span>
                    @endif
                    <a href="{{ url()->current() }}" class="shp-pill clear">Clear All</a>
                </div>
                @endif
            </div>

            {{-- Controls --}}
            <form method="GET" action="{{ url()->current() }}" id="shpCtrlForm">
            ...
            <div class="shp-grid">
                ...
                <div class="shp-card">
                    <div class="shp-thumb">
                        @if($isNew)
                            <span class="shp-disc-tag" style="color:#1F8F52;">NEW IN</span>
                    ...
                    <a href="{{ route('product-detail', $product->slug) }}" class="shp-title">{{ Str::limit($product->title, 55) }}</a>
...
$(document).on('keyup', '#shpSearch', function () {
    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            if (data.length) {
                data.forEach(item => {
                    html += `<a href="{{ url()->current() }}?search=${encodeURIComponent(item.title)}"
